...
type WorkerLister interface {
	...
	List(selector labels.Selector) (ret []*nutc.Worker, err error)
...
// List lists all Workers in the indexer.
func (s *workerLister) List(selector labels.Selector) (ret []*nutc.Worker, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*nutc.Worker))
	})
	return ret, err
}
...
type WorkerNamespaceLister interface {
	...
	Get(name string) (*nutc.Worker, error)
...
// List lists all Workers in the indexer for a given namespace.
func (s workerNamespaceLister) List(selector labels.Selector) (ret []*nutc.Worker, err error) {
...
// Get retrieves the Worker from the indexer for a given namespace and name.
func (s workerNamespaceLister) Get(name string) (*nutc.Worker, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(nutc.Resource("worker"), name)
	}
	return obj.(*nutc.Worker), nil
}
